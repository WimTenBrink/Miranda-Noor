...
const markdownToHtml = (text: string, imageUrls: string[]): string => {
    let html = text
        ...
        .replace(/```([\s\S]*?)```/g, (match, content) => `<pre class="bg-[var(--bg-inset)] p-4 rounded-md whitespace-pre-wrap font-sans text-sm text-[var(--text-secondary)] overflow-x-auto">${content.trim()}</pre>`);
